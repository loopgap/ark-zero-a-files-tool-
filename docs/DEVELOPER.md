...
为了保持仓库整洁和文档的可维护性，遵循以下规范：

- **目录划分**：
  - 根目录仅保留 `README.md` (项目总览) 和 `route.md` (运行时架构基线) 等极少量元文件。
  - 所有功能说明、开发指南、帮助文档、CI/Release 流程统一存放于 `docs/` 目录。
  - 历史报告、临时验证产物不得提交至根目录；如需保留，应放入 `docs/archive/`。
- **编写习惯**：
  - 优先使用中文（除非涉及特定技术术语或代码引用），确保团队沟通高效。
  - 保持 Markdown 标题层级清晰（# H1 仅用于文件名或顶级标题）。
  - 使用 GitHub-flavored Markdown，确保在 GitHub 网页端能正确渲染。
- **状态同步**：
  - 代码逻辑、架构或工作流发生变化时，必须同步更新 `README.md`、`docs/WIKI.md` 以及对应的详细文档。
  - 文档是项目的一部分，任何不带文档更新的功能变更均视为不完整。
