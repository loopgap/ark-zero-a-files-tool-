...
- **分支维护**：仓库长期仅保留 `main` 与 `develop` 两条常驻分支；临时分支在合并或验证完成后必须立即删除，本地与远端都不保留多余分支。
- **远端清理**：若发现旧的实验分支、自动化生成分支或一次性修复分支，必须在确认不再需要后同步从远端删除，避免分支漂移与误推送。
...
## 第十章：双系统绝对对齐保证 (Absolute Dual-System Alignment)

### 10.1 跨平台等权维护 (Equal Priority Maintenance)
- **核心承诺**：Windows 10+ 与 Debian 22.04+ 享有同等的一等公民开发权。任何功能的合入，必须在 Go 源码层实现平台条件编译（`_windows.go`, `_unix.go`）或通过 `runtime.GOOS` 显式分支。
- **无锁设计统一**：在 Windows 上的 `os.Rename` 限制与 Linux 上的原子性差异，必须通过 `SafeSave` (三段式原子落地) 在机制上抹平。

### 10.2 自动化开发流审计 (Dev-Flow Audit)
- **`scripts/dev.go` 的双重职责**：
  - `doctor`：必须同时具备检测 Windows (WebView2/Registry) 与 Linux (WebKit2GTK/pkg-config) 依赖的能力。
  - `build`：必须支持交叉编译，确保在一端可产出另一端的生产级二进制。
- **CI/CD 强制卡点**：所有合入远程仓库的代码，必须在 `scripts/dev.go test` 下全绿通过，任何导致单一系统编译失败的提交均视为无效。

### 10.3 体验与交互对齐
- **UI 一致性**：Svelte 5 确保了渲染层在 WebView2 与 WebKit2GTK 下的视觉表现一致。
- **性能对齐**：在 Debian 下的磁盘 IO 优势不得作为降低 Windows 侧性能优化的借口，确保两端 CPU 占用率在静默期均为 0%。
